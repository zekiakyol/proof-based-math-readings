---
title: "**Proof-Based Math Readings  \n Session: Algorithms**  \n 2026 Summer"
date: now
date-format: "[Version:] DD MMMM YYYY, hh:mm A"
author: "[**Zeki Akyol**](https://zekiakyol.com)^[[zekiakyol.com](https://zekiakyol.com)]  \n Department of Economics  \n Istanbul Technical University  \n [Click here for the most recent versions of the syllabuses](https://github.com/zekiakyol/proof-based-math-readings)"
pdf-engine: pdflatex
fig-dpi: 300
df-print: tibble
highlight-style: tango
knitr:
  opts_chunk: 
    collapse: true
    message: false
    fig.align: center
    comment: "#>"
format:
  pdf: 
    documentclass: article
    papersize: letter
    fontsize: 10pt
    geometry: 
      - right=1.4cm
      - left=1.4cm
      - top=1.3cm
      - bottom=1.5cm
    toc: true
    toc-depth: 3
    toccolor: blue
    number-sections: true
    colorlinks: true
    urlcolor: magenta
    citecolor: blue
    execute:
      echo: true
      warning: false
    include-in-header: 
      text: |
        \usepackage{sectsty}
        \allsectionsfont{\color{blue}}
        \setcounter{section}{-1}
        \usepackage{microtype}
        \DisableLigatures{encoding = *, family = *}
        \usepackage[fixed]{fontawesome5}
        \usepackage{float, amsmath, amssymb, amsfonts, bm, mathtools, mathastext}
        \usepackage[dvipsnames]{xcolor}
        \usepackage{hyperref}
        \hypersetup{pdfpagemode=UseNone}
editor: source
---

```{r}
#| echo: false
library(knitr)
```

```{r}
#| echo: false
#| fig-align: center
#| out-width: 40%
knitr::include_graphics("figures/proofbasedmathreadings_logo_square.png")
```

\newpage

# Motivation 

- *Proof-Based Math Readings* is a free and independent online reading group where we study mathematics required in economics master's/PhD programs using an intuitive approach.
- This session of the reading group is on *Algorithms*.

# Prerequisites

- CGPA: 3.00/4.00.
- Supplementary Python and Algorithms resources below are the prerequisites for this session.
- Please use the [\faLink Application Form](https://docs.google.com/forms/d/e/1FAIpQLSf3OxwYuAwiDXMVTCQ_x3FTtKnHmRRkl_SXlpdfqTDty06PxQ/viewform) to join our reading group.
- Applicants are informed about their application results within a week via email.

# Format

- This session takes 12 weeks.
- We discuss the topics/exercises that we struggle with at [\faDiscord Proof-Based Math Readings \[Discord\]](https://discord.com).
- We do not have face-to-face/online meetings due to the size of the group.
- Members are expected to read the chapters, and watch the chapter videos from the book's playlist.

# Resources

## Main Book and Main Book's Playlist

**Introduction to Algorithms - T. Cormen, C. Leiserson, R. Rivest, C. Stein (4th Edition, 2022)** is our main book for this session because it is well-written and well-structured.

\begin{itemize}
  \item[\faBook] Introduction to Algorithms - T. Cormen, C. Leiserson, R. Rivest, C. Stein (4th Edition, 2022)
  \item[\faYoutube] \href{https://www.youtube.com/playlist?list=PLUl4u3cNGP63EdVPNLG3ToM6LaEUuStEY}{Introduction to Algorithms - T. Cormen, C. Leiserson, R. Rivest, C. Stein (4th Edition, 2022, Playlist)}
  \item[\faBook] \href{https://mitp-content-server.mit.edu/books/content/sectbyfn/books_pres_0/11599/e4-bugs.html}{Introduction to Algorithms - T. Cormen, C. Leiserson, R. Rivest, C. Stein (4th Edition, 2022, Errata)}
  \item[\faBook] \href{https://mitp-content-server.mit.edu/books/content/sectbyfn/books_pres_0/11599/selected-solutions.pdf}{Introduction to Algorithms - T. Cormen, C. Leiserson, R. Rivest, C. Stein (4th Edition, 2022, Selected Solutions)}
\end{itemize}

## Supplementary

### Python

\begin{itemize}
  \item[\faYoutube] \href{https://www.youtube.com/playlist?list=PLUl4u3cNGP62A-ynp6v6-LGBCzeH3VAQB}{Introduction to CS and Programming using Python - Ana Bell (2022)}
\end{itemize}

### Algorithms

\begin{itemize}
  \item[\faBook] Grokking Algorithms - Aditya Bhargava (2nd Edition, 2024)
  \item[\faBook] \href{https://www.adit.io/errata.html}{Grokking Algorithms - Aditya Bhargava (2nd Edition, 2024, Errata)}
  \item[\faBook]  Data Structures and Algorithms in Python - M. T. Goodrich, R. Tamassia, M. H. Goldwasser (2013)
  \item[\faBook] \href{https://runestone.academy/ns/books/published/pythonds3/index.html}{Problem Solving with Algorithms and Data Structures using Python - Bradkey Miller, David Ranum, Roman Yasinovskyy (2023)}
  \item[\faYoutube] \href{https://www.youtube.com/@gjenkinsedu/playlists}{Problem Solving with Algorithms and Data Structures using Python - Bradkey Miller, David Ranum, Roman Yasinovskyy (2023, Playlist)}
\end{itemize}

### Proof

\begin{itemize}
  \item[\faBook] \href{https://richardhammack.github.io/BookOfProof/}{Book of Proof - Richard Hammack (3.3 Edition, 2022)}
  \item[\faYoutube] \href{https://www.youtube.com/@jeremy9959/playlists?view=50&sort=dd&shelf_id=5}{Book of Proof - Richard Hammack (3.3 Edition, 2022, Playlist by Jeremy Teitelbaum)}
  \item[\faYoutube] \href{https://www.youtube.com/playlist?list=PLVMgvCDIRy1x00m7Oo9XzEkDDACeEK_m-}{Book of Proof - Richard Hammack (3.3 Edition, 2022, Playlist by Michael Penn)}
\end{itemize}

\newpage

# Reading Schedule

- **CLRS** is the abbreviation of **Introduction to Algorithms - T. Cormen, C. Leiserson, R. Rivest, C. Stein (4th Edition, 2022)**.

::: {.callout-tip icon=false}
## **\faBook CLRS \hfill Week 01 \faCalendar* **

\begin{itemize}
  \item[] \textbf{Appendix A:} Summations
  \item[] \textbf{Appendix B:} Sets, Etc.
  \item[] \textbf{Appendix C:} Counting and Probability
  \item[] \textbf{Appendix D:} Matrices
\end{itemize}
:::

::: {.callout-tip icon=false}
## **\faBook CLRS \hfill Week 02-03 \faCalendar* **

\begin{itemize}
  \item[] \textbf{Chapter 1:} The Role of Algorithms in Computing
  \item[] \textbf{Chapter 2:} Getting Started
  \item[] \textbf{Chapter 3:} Characterizing Running Times
\end{itemize}
:::

::: {.callout-tip icon=false}
## **\faBook CLRS \hfill Week 04-05 \faCalendar* **

\begin{itemize}
  \item[] \textbf{Chapter 4:} Divide-and-Conquer
  \item[] \textbf{Chapter 5:} Probabilistic Analysis and Randomized Algorithms
\end{itemize}
:::

::: {.callout-tip icon=false}
## **\faBook CLRS \hfill Week 06-07 \faCalendar* **

\begin{itemize}
  \item[] \textbf{Chapter 6:} Heapsort
  \item[] \textbf{Chapter 7:} Quicksort
  \item[] \textbf{Chapter 8:} Sorting in Linear Time
  \item[] \textbf{Chapter 9:} Medians and Order Statistics
\end{itemize}
:::

::: {.callout-tip icon=false}
## **\faBook CLRS \hfill Week 08-09-10 \faCalendar* **

\begin{itemize}
  \item[] \textbf{Chapter 10:} Elementary Data Structures 
  \item[] \textbf{Chapter 11:} Hash Tables
  \item[] \textbf{Chapter 12:} Binary Search Trees
  \item[] \textbf{Chapter 13:} Red-Black Trees  
\end{itemize}
:::

::: {.callout-tip icon=false}
## **\faBook CLRS \hfill Week 11-12 \faCalendar* **

\begin{itemize}
  \item[] \textbf{Chapter 14:} Dynamic Programming
  \item[] \textbf{Chapter 15:} Greedy Algorithms
\end{itemize}
:::
